...
import { _coralLabelColor, _drawCoralBatchCanvas, _truncMid } from './bbox.js';
...
export function _renderCoralModelsRun(modelsRun) {
  ...
  const cards = modelsRun
    .map((m) => {
      ...
      const ms =
        m.inference_ms != null && m.inference_ms > 0
      ...
      if (!m.available) {
        ...
        body =
          ...
          m.results
            .map((b) => {
              const lat =
                b.latin && b.latin !== b.species
                  ? ` <span class="ct-species-lat">(${esc(b.latin)})</span>`
                  : '';
...
export function _renderCoralBatchResult(out, r, folder) {
  ...
  let summaryClass = 'cb-sum--ok',
    summaryIcon = '✅',
    interp = '';
  if (rate >= 0.75) {
    summaryClass = 'cb-sum--ok';
    summaryIcon = '✅';
    interp = `${modeLabel} erkennt diese Kategorie zuverlässig.`;
  } else if (rate >= 0.5) {
    summaryClass = 'cb-sum--warn';
    summaryIcon = '⚠️';
    interp = 'Mittlere Erkennungsrate — ggf. Modell oder min_score prüfen.';
  } else {
    summaryClass = 'cb-sum--bad';
    summaryIcon = '❌';
    interp = 'Niedrige Erkennungsrate — min_score zu hoch oder falsches Modell?';
  }
  ...
  const cards = results
    .map((item) => {
      ...
      if (item.wildlife) {
        ...
        const subTxt = wl.imagenet
          ? ` <span class="ct-species-lat">ImageNet: ${esc(_truncMid(wl.imagenet, 42))}</span>`
          : '';
  ...
  if (isWildlifeFolder) {
    ...
    interp =
      wlRate >= 0.75
        ? 'Wildlife-Klassifikator erkennt diese Art zuverlässig.'
        : wlRate >= 0.5
          ? 'Mittlere Klassifikationsrate — ggf. min_score prüfen.'
          : 'Niedrige Rate — Wildlife-Modell liefert oft andere Klasse.';
  }
  out.innerHTML = `
