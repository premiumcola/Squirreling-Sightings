...
export async function _loadCoralModels() {
  const list = byId('coralModelsList');
  if (!list) return;
  ...
  try {
    ...
    for (const cat of _MODEL_CATEGORIES) {
      ...
      const catNote = cat.note ? `<div class="mcat-note">${esc(cat.note)}</div>` : '';
      ...
      for (const stem of stemKeys) {
        ...
        const mkVariantHtml = (v, label) => {
          if (!v)
            return `<div class="mvar mvar--missing"><span class="mvar-kind">${esc(label)}</span><span class="mvar-empty">nicht vorhanden</span></div>`;
    ...
    list.querySelectorAll('.mvar[data-path]').forEach((card) => {
      card.addEventListener('click', async () => {
        ...
        try {
          const r = await j('/api/coral/models/select', {
            method: 'POST',
          ...
          if (r.ok) {
            ...
            // fire.
            const coralAvail = !!state.cameras?.[0]?.coral_available;
            const restartMsg = coralAvail
              ? 'Coral wird neu gestartet'
              : 'CPU-Detector wird neu gestartet';
