...
// Overlay-layer plumbing: ensure/create the SVG layers, the resize refresh,
// and the trails renderer. Reuses _positionSvgOverImage from
// live-detect-bbox-fit.js. Reads state via S.
...
// SIMU-FIX-04a · the bbox + trails layers BOTH belong inside
...
export function _ensureOverlayLayer(id, type, zIndex) {
  ...
  el.id = id;
  // H2.b Fix 3 · z-indexes 15/16 — trails below (15), bboxes on top
  // (16). Stack order is deterministic regardless of DOM insertion
  // order. Both stay under the pinned stage chrome (z 18).
  // Longhands only — NEVER the `inset` shorthand. _placeOverlay later
  // rewrites left/top/right/bottom; a stray `inset` declaration in the
  // same block re-expands to all four and silently undoes it.
  el.style.cssText =
    `position:absolute;left:0;top:0;right:auto;bottom:auto;` +
    `width:100%;height:100%;pointer-events:none;z-index:${zIndex}`;
  host.appendChild(el);
  return el;
}
