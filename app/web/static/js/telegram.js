...
async function refreshTelegramPollingStatus() {
  ...
  try {
    ...
    /* leave the existing badge alone on transient fetch errors */
  }
...
export function renderTgFormatPreview(fmt) {
  ...
  if (fmt === 'video')
    html += `<div class="tg-bubble-vid">🎬 Video-Clip angehängt (wenn verfügbar)</div>`;
...
byId('telegramTestBtn')?.addEventListener('click', async () => {
  ...
  try {
    ...
    btn.disabled = false;
    btn.textContent = '📨 Testnachricht senden';
    if (res)
      setTimeout(() => {
