...
export function _renderWhitelistChips(profiles, selected) {
  ...
  el.innerHTML = profiles
    .map(
      (p) =>
        `<span class="wl-chip ${_whitelistState.includes(p.name) ? 'selected' : ''}" onclick="toggleWlChip('${esc(p.name)}')">${esc(p.name)}</span>`,
    )
    .join('');
  _updateWhitelistHidden();
}
...
window.toggleWlChip = function (name) {
  ...
  document
    ...
    .forEach((c) => c.classList.toggle('selected', _whitelistState.includes(c.textContent)));
  _updateWhitelistHidden();
};
