...
function _renderLines(traceLines, detCount, passCount) {
  ...
  bodyEl.innerHTML = all
    .map((line) => `<div class="erk-sim-log-line ${_classifyLine(line)}">${esc(line)}</div>`)
    .join('');
}
...
function _wireToggle(camId) {
  ...
  if (!toggle.dataset.wired) {
    ...
    toggle.addEventListener('click', () => {
      ...
      try {
        ...
        /* private mode / quota — silently keep session-only */
      }
    });
  }
  ...
  let stored = null;
  try {
    stored = camId ? localStorage.getItem(_LS_KEY_PREFIX + camId) : null;
  } catch {
    /* ignore */
  }
  _applyOpen(stored === '1');
