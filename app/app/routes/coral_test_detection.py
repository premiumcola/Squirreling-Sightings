...
# SIMU-07 · debug-snapshot endpoint. Returns a self-contained document
# of the camera's current live state — everything a debugging session
# needs in one paste-able blob, so the operator never has to open a root
# shell on the phone. Reads the per-cam test-tracker state (no fresh
# inference) so it is cheap and reflects the LAST tick the user saw.
# ``?format=json`` additionally returns the machine-readable findings
# list the Debug tab renders on screen; the default stays Markdown.
# Assembly lives in _debug_snapshot.py (this file is already long).
