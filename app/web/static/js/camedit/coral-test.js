...
import { _renderCoralBatchResult, _renderCoralModelsRun } from './coral-test/results.js';
...
export { _renderCoralPipelineTree, _updateCoralDeviceInfo };
...
window._toggleCoralSetting = async function (key, inputEl) {
  ...
  const birdEnabled =
    key === 'bird_species_enabled' ? nowOn : !!byId('birdSpeciesEnabled')?.checked;
  const wildlifeOn = key === 'wildlife_enabled' ? nowOn : !!byId('wildlifeEnabled')?.checked;
  await apiPost('/api/settings/app', {
    processing: {
      coral_enabled: coralEnabled,
      bird_species_enabled: birdEnabled,
      wildlife_enabled: wildlifeOn,
    },
  });
...
window.toggleCoralTab = function (tabId) {
  document
    .querySelectorAll('.coral-tab')
    .forEach((t) => t.classList.toggle('active', t.dataset.tab === tabId));
  document
    .querySelectorAll('.coral-tab-content')
    .forEach((p) => (p.hidden = p.id !== `coral-pane-${tabId}`));
...
export async function _populateCoralTestCameras() {
  const sel = byId('coralTestCamSel');
  if (!sel) return;
  ...
  let folders = [];
  try {
    const r = await j('/api/coral/test-images');
    folders = r.folders || [];
  } catch {}
  ...
  if (folders.length) {
    html +=
      '<optgroup label="— Testbilder —">' +
      folders
        .map(
          (f) =>
            `<option value="test:${esc(f.name)}">${esc(f.icon || '📁')} ${esc(f.label || f.name)} (${f.count} Bilder)</option>`,
        )
        .join('') +
      '</optgroup>';
  }
...
async function _runCoralTest() {
  const btn = byId('coralTestBtn');
  const out = byId('coralTestResult');
  if (!btn || !out) return;
  ...
  if (sel.startsWith('test:')) {
    ...
    try {
      const r = await j('/api/coral/test-batch', {
        method: 'POST',
  ...
  try {
    const r = await j('/api/coral/test', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ camera_id: camId || undefined }),
    });
    const mode = r.detector_mode || 'motion_only';
    const modeColor =
      mode === 'coral' ? '#4ade80' : mode === 'cpu' ? '#facc15' : 'rgba(255,255,255,0.6)';
    const modeLabel =
      mode === 'coral' ? '⚡ Coral TPU' : mode === 'cpu' ? '💻 CPU-Fallback' : '⏸ Bewegung';
    ...
    const pillsHtml = dets.length
      ? dets
          .map((d) => {
            ...
            const spLat =
              d.species_latin && d.species && d.species !== d.species_latin
    ...
    const reasonRow =
      r.detector_reason && r.detector_reason !== 'ok'
        ? `<div class="field-help" style="margin-top:6px;font-family:monospace">${esc(r.detector_reason)}</div>`
        : '';
    const usbRow = r.usb_info
      ? `<div class="field-help" style="margin-top:4px;color:#a78bfa">🔌 ${esc(r.usb_info)}</div>`
      : '';
    const errRow = r.inference_error
      ? `<div style="color:#fca5a5;margin-top:6px;font-size:12px">Inferenz-Fehler: ${esc(r.inference_error)}</div>`
      : '';
