...
async function _openLightboxByEventId(eventId) {
  ...
  let meta = null;
  try {
    meta = await j(`/api/event/${encodeURIComponent(eventId)}`);
  } catch {}
...
window.addEventListener('load', () => {
  setTimeout(_routeFromHash, 1500);
});
