...
function _renderStatistik(monthData, dayData) {
  ...
  const top3 = Object.entries(labelCounts)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 3);
  ...
  const hmData = {};
  cameras.forEach((c) => {
    hmData[c.id] = new Array(24).fill(0);
  });
  ...
  const hmColLabels = Array.from({ length: 24 }, (_, col) =>
    new Date(nowMs - (23 - col) * 3600000).getHours(),
  );

  const periodPills = [
    ['Heute', todayCount],
    ['Diese Woche', weekCount],
    ['Dieser Monat', monthCount],
  ]
    .map(
      ([label, count]) =>
        `<div class="stat-period-pill"><div class="stat-period-num">${count}</div><div class="stat-period-label">${label}</div></div>`,
    )
    .join('');

  // Slice + swatch colour comes from the camera icon (getCameraColor) so the
  // chart matches the icon next to each legend label.
  const camsWithEvents = cameras
    .map((c) => ({ c, cnt: camCounts[c.id] || 0 }))
    .filter((x) => x.cnt > 0);
  ...
  const topLabels = topN.length
    ? topN
        .map(([label, cnt]) => {
          ...
          const color =
            CAT_COLORS[label] || colors[label] || _STAT_LABEL_COLORS[label] || '#cbd5e1';
          ...
          const lblClick = STAT_MEDIA_DRILLDOWN
            ? `onclick="_statOpenMedia('','${esc(label)}')"`
            : '';
    ...
    ? `<div class="stat-heatmap-wrap"><div class="stat-hm-grid">
        ...
        ${cameras
          .map((c) => {
            ...
            const hmCamClick = STAT_MEDIA_DRILLDOWN
              ? `onclick="_statOpenMedia('${esc(c.id)}','')"`
              : '';
            const cells = hmColLabels
              .map((labelHour, col) => {
                const hoursAgo = 23 - col;
                const cnt = hours[hoursAgo] || 0;
                const alpha = cnt === 0 ? 0 : Math.max(0.18, 0.12 + (cnt / hmMax) * 0.6);
                const bg =
                  cnt === 0 ? 'rgba(255,255,255,0.04)' : `rgba(255,255,255,${alpha.toFixed(2)})`;
  ...
  if (hmBlock)
    hmBlock.innerHTML = `<div class="stat-card" style="margin-top:16px"><div class="stat-card-title">${_STAT_TITLE_SVG.grid}<span>Letzte 24h · Aktivität nach Stunde</span></div>${heatmap}</div>`;
  ...
  if (!_hmTip) {
    ...
    _hmTip.style.cssText =
      'position:fixed;z-index:9999;background:#0d1422;color:#edf4fb;font-size:11px;font-weight:600;padding:4px 9px;border-radius:8px;white-space:nowrap;pointer-events:none;box-shadow:0 2px 10px rgba(0,0,0,.6);display:none;border:1px solid rgba(255,255,255,.08)';
...
byId('statRefreshBtn')?.addEventListener('click', () => {
  _statLoaded = false;
  loadStatistik();
});
