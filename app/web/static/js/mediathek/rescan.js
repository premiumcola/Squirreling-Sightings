...
function _showFixThumbsBar(done, total, finalMsg) {
  ...
  if (!bar) {
    ...
    bar.id = 'fixThumbsBar';
    bar.style.cssText =
      'position:fixed;bottom:0;left:0;right:0;z-index:500;background:var(--panel);border-top:1px solid rgba(255,255,255,.08);font-size:13px;color:var(--text)';
    bar.innerHTML = `
    ...
    try {
      ...
      if (det && Array.isArray(s.recent)) {
        s.recent.forEach((fname) => {
          ...
          line.style.cssText =
            'padding:3px 0;border-bottom:1px solid rgba(255,255,255,.04);white-space:nowrap;overflow:hidden;text-overflow:ellipsis';
      ...
      if (!s.running) {
        ...
        const msg =
          errs > 0
            ? `✓ ${done - errs} Thumbnails erzeugt, ${errs} Fehler`
            : `✓ ${done} Thumbnails erzeugt`;
...
byId('reindexTrackingBtn')?.addEventListener('click', async () => {
  ...
  try {
    const url =
      '/api/tracking/reindex-all' + (camId ? `?camera_id=${encodeURIComponent(camId)}` : '');
    let d;
    try {
      d = await apiPost(url);
    } catch (e) {
    ...
    showToast(
      `${queued} Clip${queued === 1 ? '' : 's'} werden indiziert (${scopeLabel}) …`,
      'info',
    );
    ...
    const tick = async () => {
      ...
      let s;
      try {
        s = await apiGet('/api/tracking/status');
      } catch {
        s = null;
      }
      if (!s || !s.alive) {
