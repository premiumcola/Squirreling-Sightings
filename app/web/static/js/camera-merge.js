...
export function openMergeModal(sourceId, sourceName) {
  ...
  byId('mergeIntro').innerHTML =
    `Medien von <strong>${esc(sourceName)}</strong> werden in die gewählte Ziel-Kamera verschoben. Der Eintrag <strong>${esc(sourceName)}</strong> wird danach gelöscht.`;
  const list = byId('mergeTargets');
  if (!targets.length) {
    list.innerHTML =
      '<div class="item muted" style="padding:12px">Keine aktive Ziel-Kamera verfügbar.</div>';
  } else {
...
export function bindMergeModal() {
  ...
  byId('mergeModal')?.addEventListener('click', (e) => {
    if (e.target === byId('mergeModal')) closeMergeModal();
  });
  ...
  byId('mergeConfirmBtn')?.addEventListener('click', async () => {
    ...
    try {
      const r = await j(
        `/api/cameras/${encodeURIComponent(_mergeSource.id)}/merge-into/${encodeURIComponent(_mergeTarget.id)}`,
        { method: 'POST' },
      );
      const tgtName = _mergeTarget.name;
      closeMergeModal();
      showToast(
        `${r.moved_files || 0} Datei(en) nach „${tgtName}“ verschoben (${r.moved_events || 0} Events, ${r.moved_timelapses || 0} Timelapse).`,
        'success',
      );
