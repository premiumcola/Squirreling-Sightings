...
function axisLabelsSvg(axes, cx, cy, r, interactive) {
  return axes
    .map((axis, i) => {
      ...
      const attrs = interactive ? ` type="button" data-axis-label="${esc(axis.label)}"` : '';
      return (
...
// ── settings (Fangnetz primary) radar ──────────────────────────────────
// Same geometry (rings, spokes, polygon, radial hit targets) as the
// confidence radar above, but a plain vertex encoding: tune axes carry
// no Telegram evidence, so there is no ready-ring, no evidence-sized
// vertex, no dashed learner proposal — only werk (untouched) vs manuell
// (operator moved it), the whole of their provenance. Text labels, no
// icon glyph: these are settings, not object classes, and inventing
// icons for "Analyse-Intervall" would be worse than no icon.

const TUNE_WERK = 'rgba(255,255,255,.30)';

function _tuneVertexSvg(axis, i, n, cx, cy, r, interactive) {
  const p = polar(i, n, radiusForE(axis.E, r), cx, cy);
  const manuell = axis.provenance === 'manuell';
  const fill = manuell ? '#7ac8ff' : 'none';
  const stroke = manuell ? '#7ac8ff' : TUNE_WERK;
  const hit = interactive
    ? `<circle class="netz-tune-hit" data-tune-axis="${esc(axis.key)}" cx="${p.x.toFixed(1)}" ` +
      `cy="${p.y.toFixed(1)}" r="22" fill="transparent"/>`
    : '';
  return (
    `<g class="netz-vertex" data-tune-axis="${esc(axis.key)}">` +
    `<circle cx="${p.x.toFixed(1)}" cy="${p.y.toFixed(1)}" r="5.5" ` +
    `fill="${fill}" stroke="${stroke}" stroke-width="1.5"/>${hit}</g>`
  );
}

function _tuneSpokesSvg(axes, cx, cy, r) {
  return axes
    .map((axis, i) => {
      const p = polar(i, axes.length, r, cx, cy);
      return (
...
function _tuneLabelsSvg(axes, cx, cy, r, interactive) {
  return axes
    .map((axis, i) => {
      const p = polar(i, axes.length, r + 26, cx, cy);
      const tag = interactive ? 'button' : 'span';
      ...
      return (
        `<foreignObject x="${(p.x - 38).toFixed(1)}" y="${(p.y - 18).toFixed(1)}" ` +
        `width="76" height="36">` +
        `<${tag} xmlns="http://www.w3.org/1999/xhtml" class="netz-axis-lbl netz-axis-lbl--tune"${attrs}>` +
        `<span class="netz-axis-name">${esc(axis.label)}</span>` +
        `</${tag}></foreignObject>`
      );
    })
    .join('');
...
/**
 * @param {object} opts
 * @param {Array}  opts.axes   rows from buildTuneAxes(): {key, label, E, provenance}
 * @param {number} opts.side
 * @param {boolean} opts.interactive
 */
export function renderTuneRadar({ axes, side, interactive = true }) {
  const cx = side / 2;
  const cy = side / 2;
  const r = chartRadius(side);
  ...
  return (
    `<svg class="netz-svg" viewBox="0 0 ${side} ${side}" width="${side}" height="${side}" ` +
    `role="img" aria-label="Fangnetz">` +
    ringsSvg(cx, cy, r) +
    _tuneSpokesSvg(axes, cx, cy, r) +
    ringLabelsSvg(cx, cy, r) +
    polygonSvg(pointsFor(axes, values, cx, cy, r)) +
    axes.map((a, i) => _tuneVertexSvg(a, i, axes.length, cx, cy, r, interactive)).join('') +
    _tuneLabelsSvg(axes, cx, cy, r, interactive) +
    `</svg>`
  );
}
...
export function legendHtml(axes) {
  const dots = [
    ...
    .map(
      ([t, c]) => `<span class="netz-leg-dot"><i style="background:${esc(c)}"></i>${esc(t)}</span>`,
    )
    .join('');
