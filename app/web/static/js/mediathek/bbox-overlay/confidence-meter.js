...
function _buildMeterRow(label, valFrac, thresholdFrac, color, opts = {}) {
  ...
  const fillColor =
    opts.amberBelowThreshold && thresholdFrac != null && valFrac < thresholdFrac
      ? _GAUGE_AMBER
      : color;
  return `
    <div class="lbcm-row">
    ...
    .map(({ track, sample, num }) => {
      ...
      rows.push(
        _buildMeterRow(
          'Score',
          sample.score || 0,
          scoreThresh != null ? parseFloat(scoreThresh) : null,
          c,
      ...
      if (floors) {
        ...
        if (floors.min_h_frac > 0) {
          rows.push(_buildMeterRow('Bbox-Höhe', fracH, null, c, { showTick: false }));
        }
        if (floors.min_area_frac > 0) {
          rows.push(_buildMeterRow('Bbox-Fläche', fracArea, null, c, { showTick: false }));
        }
      }
